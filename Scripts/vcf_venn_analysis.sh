...
vcf_intersect.py  --v --filter PASS   1.diff.2.pass.snps.vcf   2.filtered.snps.vcf   | grep -v \# | awk '{print $1 "\t" $2-1 "\t" $2}' > $VCF1unique.uniq.bed
wc -l $VCF1unique.uniq.bed
...
vcf_intersect.py  --v --filter PASS   2.diff.1.pass.snps.vcf  1.filtered.snps.vcf   | grep -v \# | awk '{print $1 "\t" $2-1 "\t" $2}' > $VCF2unique.uniq.bed
wc -l $VCF2unique.uniq.bed
